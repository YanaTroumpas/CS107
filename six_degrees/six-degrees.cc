...
// initialize queue with source (first actor)
vector<film> sourceCredits;
...
while(!partialPaths.empty() && partialPaths.front().getLength() <= 5){
  ...
  partialPaths.pop();
  vector<film> movies;
  db.getCredits(currentPath.getLastPlayer(), movies);

  previouslySeenActors.insert(currentPath.getLastPlayer());
  ...
  for(auto movie:movies){
    if(!previouslySeenFilms.count(movie)){
      ...
      for(const string& actor:actors){
        if(!previouslySeenActors.count(actor)){
          previouslySeenActors.insert(actor);
          path cloned_path = currentPath;
          cloned_path.addConnection(movie, actor);
          if(actor == target){
            cout << cloned_path << endl;
            return;
          }
          partialPaths.push(cloned_path);
        }
      }
    }
  }
